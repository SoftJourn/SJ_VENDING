...
public class CoinService {
  ...
  public TransactionDTO spent(Principal principal, BigDecimal amount, String machineAddress) {
    return wrapExceptionHandling(() -> {
      ResponseEntity<TransactionDTO> response = coinRestTemplate
          .exchange(coinsServerHost + "/buy/" + machineAddress,
              HttpMethod.POST,
      ...
      if (response.getBody().getStatus().equals(SUCCESS_TRANSACTION_STATUS)) {
        ...
        throw new PaymentProcessingException(
            "Unsuccessful call to coins server. " + response.getBody().getError());
      }
    });
  }
  ...
  public void returnMoney(TransactionDTO tx) {
    ...
    ResponseEntity<TransactionDTO> response = oAuthHelper
        .requestWithToken(url, HttpMethod.POST, HttpEntity.EMPTY, TransactionDTO.class);
    if (! response.getStatusCode().is2xxSuccessful()) {
      throw new PaymentProcessingException(
          "Unable to rollback transaction "
              + tx.getId() + ". Coins server response " + response.toString());
    }
    if (!response.getBody().getStatus().equals(SUCCESS_TRANSACTION_STATUS)) {
      throw new PaymentProcessingException(
          "Unable to rollback transaction "
              + tx.getId() + ". Coins server error " + response.getBody().getError());
    }
  }
  ...
  private void distributeMoney(Principal adminPrincipal, BigDecimal amount, String machineName) {
    wrapExceptionHandling(() -> {
      ResponseEntity<TransactionDTO> response = coinRestTemplate
          .exchange(coinsServerHost + "/distribute/" + machineName,
              HttpMethod.POST,
