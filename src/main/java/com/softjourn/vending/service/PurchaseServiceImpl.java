  ...
  public Page<PurchaseDTO> getAllUsingFilter(PurchaseFilterDTO filter, Pageable pageable) {
    ...
    if (filter.getMachineId() == -1) {
      ...
      if (filter.getType().equals(PurchaseDateEnum.Any.getType())) {
        ...
        return purchaseConverter(purchaseRepository
            .findAllByStartDue(getStartDate(LocalDate.now().toString(), filter.getTimeZoneOffSet()),
        ...
        return purchaseConverter(purchaseRepository
            .findAllByStartDue(getStartDate(getPastDate(LAST_WEEK), filter.getTimeZoneOffSet()),
                getDueDate(LocalDate.now().toString(), filter.getTimeZoneOffSet()), pageable));
      } // by last month
      else if (filter.getType().equals(PurchaseDateEnum.LastMonth.getType())) {
        return purchaseConverter(purchaseRepository
            .findAllByStartDue(getStartDate(getPastDate(LAST_MONTH), filter.getTimeZoneOffSet()),
                getDueDate(LocalDate.now().toString(), filter.getTimeZoneOffSet()), pageable));
      } // by Start due
      else if (filter.getType().equals(PurchaseDateEnum.StartDue.getType())) {
        return purchaseConverter(purchaseRepository
            .findAllByStartDue(getStartDate(filter.getStart(), filter.getTimeZoneOffSet()),
      ...
      if (filter.getType().equals(PurchaseDateEnum.Any.getType())) {
        return purchaseConverter(purchaseRepository
            .findAllByMachineIdOrderByTimeDesc(filter.getMachineId(), pageable));
      } // by today's date
      else if (filter.getType().equals(PurchaseDateEnum.Today.getType())) {
        return purchaseConverter(purchaseRepository.findAllByStartDue(
            filter.getMachineId(),
            getStartDate(LocalDate.now().toString(), filter.getTimeZoneOffSet()),
            getDueDate(LocalDate.now().toString(), filter.getTimeZoneOffSet()), pageable));
      } // by last week
      else if (filter.getType().equals(PurchaseDateEnum.LastWeek.getType())) {
        return purchaseConverter(purchaseRepository.findAllByStartDue(
            filter.getMachineId(), getStartDate(getPastDate(LAST_WEEK), filter.getTimeZoneOffSet()),
            getDueDate(LocalDate.now().toString(), filter.getTimeZoneOffSet()), pageable));
      } // by last month
      else if (filter.getType().equals(PurchaseDateEnum.LastMonth.getType())) {
        return purchaseConverter(purchaseRepository.findAllByStartDue(
            filter.getMachineId(),
            getStartDate(getPastDate(LAST_MONTH), filter.getTimeZoneOffSet()),
            getDueDate(LocalDate.now().toString(), filter.getTimeZoneOffSet()), pageable));
      } // by Start due
      else if (filter.getType().equals(PurchaseDateEnum.StartDue.getType())) {
        return purchaseConverter(purchaseRepository.findAllByStartDue(
            filter.getMachineId(), getStartDate(filter.getStart(), filter.getTimeZoneOffSet()),
            getDueDate(filter.getDue(), filter.getTimeZoneOffSet()), pageable));
      }
    }
    return null;
  ...
  public List<SoldProductDTO> getTopProductsByTimeRange(Integer topSize, String start, String due) {
    try {
      ...
      throw new IllegalArgumentException(
          "Datetime field should be in ISO format(Example: 2016-10-06T04:00:00Z)");
    }
  }
